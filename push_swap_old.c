...
/*   Updated: 2025/01/13 20:16:23 by obouizi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int get_next_in_chunk(t_list *stack, int *sorted_arr, int start, int end)
{
    ...
    while (current)
    {
        if (current->num >= sorted_arr[start] && current->num < sorted_arr[end])
            return index;  // Found the element in the chunk
        index++;
...
void sort_stack_b(t_list **stack_b)
{
    ...
    if ((*stack_b)->num < (*stack_b)->next->num)
        swap_b(*stack_b);  // Keep top elements sorted
}
...
void bring_to_top(t_list **stack, int index)
{
    ...
    {
        while (index-- > 0)
            rotate_a(stack);  // Rotate towards top
    }
    else
    {
        index = size - index;  // Calculate how many rotations in reverse
        while (index-- > 0)
            reverse_rotate_a(stack);  // Rotate in reverse
    }
}

void push_chunk_to_b(t_list **stack_a, t_list **stack_b, int *sorted_arr, int start, int end)
{
    int index;

    while ((index = get_next_in_chunk(*stack_a, sorted_arr, start, end)) != -1)
    {
...
void push_chunks(t_list **stack_a, t_list **stack_b, int *sorted_arr, int total_size)
{
    int chunk_size = total_size / 5;
    int start = 0;
    int end = chunk_size;

    while (start < total_size)
    {
        if (end > total_size)  // Ensure we don’t go out of bounds
            end = total_size;

        push_chunk_to_b(stack_a, stack_b, sorted_arr, start, end);
...
void sort(t_list **stack_a, t_list **stack_b)
{
	...
	{
		push_chunks(stack_a, stack_b, sorted_version(*stack_a), ft_lstsize(*stack_a));
